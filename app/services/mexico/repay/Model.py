...
class Account(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('uk_individual_no_owner', 'account_user_id_num_encrypt', 'account_owner'),
        db.Index('idx_acc_no', 'account_user_id_num_encrypt', 'account_no')
    )

    account_id = db.Column(db.BigInteger, primary_key=True, info='账户ID')
    account_no = db.Column(db.String(20), nullable=False, info='账户业务主键')
    ...
    account_user_id_num_encrypt = db.Column(db.String(128), nullable=False, info='身份证密文')
...
class AccountRecharge(db.Model, BaseToDict):
    ...
    account_recharge_source_type = db.Column(db.Enum('withhold', 'singleToMultiple', 'offline', 'coupon', 'provision'), nullable=False, server_default=db.FetchedValue(), info='数据来源：withhold(代扣)、singleToMultiple(单转多)、offline(线下)、coupon(优惠券)')
    account_recharge_serial_no = db.Column(db.String(64), nullable=False, unique=True, server_default=db.FetchedValue(), info='充值流水号，代扣 使用代扣流水号，其他没有由系统自己生产')
...
class AccountRepay(db.Model, BaseToDict):
    ...
    account_repay_account_no = db.Column(db.BigInteger, nullable=False, index=True, info='账户编号')
...
class AccountStatementRecord(db.Model, BaseToDict):
    __tablename__ = 'account_statement_record'
    __table_args__ = (
        db.Index('idx_card_number', 'account_statement_record_account_number', 'account_statement_record_side_account_number'),
        db.Index('idx_item_no', 'account_statement_record_item_no', 'account_statement_record_create_at')
    )

    account_statement_record_id = db.Column(db.Integer, primary_key=True, info='主键')
    account_statement_record_trade_id = db.Column(db.Integer, nullable=False, unique=True, info='唯一交易流水ID')
    account_statement_record_trade_date = db.Column(db.DateTime, nullable=False, info='交易时间')
    account_statement_record_account_number = db.Column(db.String(64), nullable=False, info='我方账户号码-我方收款卡')
    account_statement_record_side_account_number = db.Column(db.String(64), nullable=False, info='对方账号号码-用户还款卡')
    account_statement_record_in_amount = db.Column(db.BigInteger, nullable=False, server_default=db.FetchedValue(), info='转入金额')
    account_statement_record_out_amount = db.Column(db.BigInteger, nullable=False, server_default=db.FetchedValue(), info='转出金额')
    account_statement_record_balance = db.Column(db.BigInteger, nullable=False, server_default=db.FetchedValue(), info='余额')
    account_statement_record_currency = db.Column(db.String(32), nullable=False, info='币种，CNY: 人民币, USD: 美元,INR: 印度卢比,THB: 泰国铢')
    account_statement_record_status = db.Column(db.Enum('init', 'match_success', 'match_fail'), server_default=db.FetchedValue(), info='状态')
    account_statement_record_item_no = db.Column(db.String(128), info='匹配的资产编号')
    account_statement_record_serial_no = db.Column(db.String(128), index=True, info='匹配的代扣流水号')
    account_statement_record_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
    account_statement_record_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='更新时间')
    account_statement_record_payment_card_number = db.Column(db.String(256), server_default=db.FetchedValue(), info='用户确认的还款卡')
    account_statement_record_borrow_uuid = db.Column(db.String(256), info='用户的borrowId')
    account_statement_record_check_row = db.Column(db.String(128), index=True, info='流水唯一字段 md5(账号+日期+流入+流出+余额等)')
...
class Arbitration(db.Model, BaseToDict):
    ...
    arbitration_status = db.Column(db.Enum('nofinish', 'finish', 'payoff'), nullable=False, server_default=db.FetchedValue(), info='仲裁状态')
    ...
    arbitration_user_id_num_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='身份证 密文字段')
    arbitration_user_name_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='仲裁用户姓名 密文字段')
...
class Asset(db.Model, BaseToDict):
    ...
    asset_late_amount = db.Column(db.BigInteger, server_default=db.FetchedValue(), info='包含所有的逾期费用+授信费')
    asset_due_bill_no = db.Column(db.String(50), index=True, info='外部编号')
    ...
    asset_decrease_late_amount = db.Column(db.BigInteger, server_default=db.FetchedValue(), info='减免逾期费用')
    asset_from_app = db.Column(db.String(30))
    asset_repayment_app = db.Column(db.String(10), nullable=False, server_default=db.FetchedValue())


class AssetDelay(db.Model, BaseToDict):
    __tablename__ = 'asset_delay'
    __table_args__ = (
        db.Index('idx_asset_delay_withhold_serial_no', 'asset_delay_withhold_serial_no', 'asset_delay_item_no', 'asset_delay_period'),
    )

    asset_delay_id = db.Column(db.BigInteger, primary_key=True, info='展期记录自增长id')
    asset_delay_trade_no = db.Column(db.String(64), nullable=False, index=True, info='展期订单编号')
    asset_delay_withhold_serial_no = db.Column(db.String(64), nullable=False, info='展期代扣序列号')
    asset_delay_item_no = db.Column(db.String(64), nullable=False, index=True, info='展期资产编号')
    asset_delay_period = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='展期期次')
    asset_delay_days = db.Column(db.Integer, nullable=False, info='展期天数')
    asset_delay_status = db.Column(db.Enum('open', 'success', 'fail', 'refund'), nullable=False, server_default=db.FetchedValue(), info='展期状态')
    asset_delay_pay_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='展期支付完成时间')
    asset_delay_start_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='展期开始时间')
    asset_delay_end_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='展期结束时间')
    asset_delay_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='展期记录创建时间')
    asset_delay_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='展期记录更新时间')
...
class AssetFoxLogDetail(db.Model, BaseToDict):
    ...
    asset_fox_log_detail_item_no = db.Column(db.String(48), nullable=False, index=True)
    asset_fox_log_detail_action_type = db.Column(db.String(255))
...
class AssetLateFeeRefreshLog(db.Model, BaseToDict):
    ...
    asset_late_fee_refresh_log_asset_item_no = db.Column(db.String(48), nullable=False, index=True, info='Asset id')
...
class AssetOperationAuth(db.Model, BaseToDict):
    ...
    asset_operation_auth_action = db.Column(db.Enum('withhold', 'other', 'withholdAuto', 'refreshFee', 'createWithholdAutoTask'), nullable=False, info='操作类型')
...
class AssetTran(db.Model, BaseToDict):
    ...
    __table_args__ = (
        ...
        db.Index('Idx_asset_tran_asset_item_no_tran_type', 'asset_tran_asset_item_no', 'asset_tran_type')
    )
    ...
    asset_tran_period = db.Column(db.Integer, nullable=False, index=True, info='期次')
    ...
    asset_tran_repay_priority = db.Column(db.SmallInteger, nullable=False, info="逾期费用的状态，'normal' 正常，'max'已达到最大")
    asset_tran_trade_at = db.Column(db.DateTime, nullable=False, index=True, info='交易时间')
    ...
    asset_tran_update_at = db.Column(db.DateTime, nullable=False, index=True, server_default=db.FetchedValue())
...
class AssetTranLog(db.Model, BaseToDict):
    ...
    asset_tran_log_from_system = db.Column(db.Enum('rbiz', 'biz', 'cmdb', 'fox', 'dsq'), nullable=False)
    ...
    asset_tran_log_ref_no = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='引用字段的编号')
...
class Buyback(db.Model, BaseToDict):
    ...
    buyback_start_date = db.Column(db.Date, nullable=False, info='回购开始日期')
...
class CapitalAsset(db.Model, BaseToDict):
    ...
    capital_asset_granted_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='资方起息时间')
    ...
    capital_asset_version = db.Column(db.BigInteger, nullable=False, server_default=db.FetchedValue())
...
class CapitalConfig(db.Model, BaseToDict):
    ...
    capital_config_code = db.Column(db.String(16), nullable=False, unique=True, info='渠道code')
    capital_config_name = db.Column(db.String(32), nullable=False, info='渠道名称')
    capital_config_status = db.Column(db.Enum('valid', 'invalid'), server_default=db.FetchedValue(), info='状态：valid 有效：invalid无效')
    capital_config_open_channel = db.Column(db.Enum('Y', 'N'), server_default=db.FetchedValue(), info='是否启用该通道 Y 启用，N 不启用')
    capital_config_create_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='创建时间')
    capital_config_update_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='更新时间')
    capital_config_need_split = db.Column(db.String(1), server_default=db.FetchedValue(), info='是否需要拆单，默认为N')
...
class CapitalNotify(db.Model, BaseToDict):
    ...
    capital_notify_withhold_result_serial_no = db.Column(db.String(50), info='执行代扣流水号')
    ...
    capital_notify_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
...
class CapitalRepay(db.Model, BaseToDict):
    __tablename__ = 'capital_repay'

    capital_repay_id = db.Column(db.BigInteger, primary_key=True)
    capital_notify_asset_item_no = db.Column(db.String(64), nullable=False, index=True, info='资产编号')
    capital_notify_period_start = db.Column(db.Integer, nullable=False, info='还款起始期次')
    capital_notify_period_end = db.Column(db.Integer, nullable=False, info='还款结束期次')
    capital_notify_loan_channel = db.Column(db.String(32), nullable=False, info='放款渠道')
    capital_notify_withhold_result_serial_no = db.Column(db.String(50), info='执行代扣流水号')
    capital_notify_status = db.Column(db.Enum('open', 'process', 'sucess', 'fail'), nullable=False, server_default=db.FetchedValue(), info='推送状态:')
    capital_notify_push_info = db.Column(db.Text, info='推送信息-根据资金方通知需求，记录计算明细，便于通知失败排查问题')
    capital_notify_req_data = db.Column(db.Text, info='请求消息体内容')
    capital_notify_res_data = db.Column(db.Text, info='接口返回结果内容')
    capital_notify_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
    ...
    capital_notify_plan_at = db.Column(db.DateTime, nullable=False, index=True, info='计划发送时间')
    capital_notify_res_msg = db.Column(db.Text)
    capital_notify_action_type = db.Column(db.Enum('overdue', 'repay'), nullable=False, info='overdue：逾期推送,repay：还款推送')



class CapitalRepayNotify(db.Model, BaseToDict):
    __tablename__ = 'capital_repay_notify'

    capital_repay_notify_id = db.Column(db.BigInteger, primary_key=True, nullable=False)
    capital_repay_notify_asset_item_no = db.Column(db.String(64), nullable=False, index=True, info='资产编号')
    capital_repay_notify_repay_period_start = db.Column(db.Integer, nullable=False, info='还款起始期次')
    capital_repay_notify_repay_period_end = db.Column(db.Integer, nullable=False, info='还款结束期次')
    capital_repay_notify_loan_channel = db.Column(db.String(32), nullable=False, info='放款渠道')
    capital_repay_notify_withhold_result_serial_no = db.Column(db.String(50), nullable=False, info='执行代扣流水号')
    capital_repay_notify_actual_repay_time = db.Column(db.DateTime, nullable=False, info='用户实际还款时间')
    capital_repay_notify_status = db.Column(db.Enum('open', 'process', 'sucess', 'fail'), primary_key=True, nullable=False, server_default=db.FetchedValue(), info='推送状态:')
    capital_repay_notify_push_info = db.Column(db.Text, info='推送信息-根据资金方通知需求，记录计算明细，便于通知失败排查问题')
    capital_repay_notify_req_data = db.Column(db.Text, info='请求消息体内容')
    capital_repay_notify_res_data = db.Column(db.Text, info='接口返回结果内容')
    capital_repay_notify_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
    capital_repay_notify_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='更新时间')
    capital_repay_notify_plan_at = db.Column(db.DateTime, nullable=False, index=True, info='计划发送时间')
    capital_repay_notify_res_msg = db.Column(db.Text)
...
class CapitalRepayTran(db.Model, BaseToDict):
    ...
    capital_repay_tran_status = db.Column(db.Enum('ready', 'success', 'fail', 'process', 'cancel'), info='read :就绪 、process：处理中、fail：失败、success：成功')
    capital_repay_tran_id_card_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='身份证 密文')
    capital_repay_tran_bank_card_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='银行卡 密文')
    capital_repay_tran_real_name_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='姓名 密文')
    capital_repay_tran_phone_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='手机 密文')
...
class CapitalRoute(db.Model, BaseToDict):
    ...
    capital_route_config_code = db.Column(db.String(16), info='代扣渠道编码')
    capital_route_status = db.Column(db.String(12), info='状态  有效:valid 无效: invalid')
    capital_route_conditional = db.Column(db.String(32), info='条件code')
    capital_route_conditional_value = db.Column(db.String(128), info='取值')
    capital_route_change_pay_server = db.Column(db.String(1), info='是否切换payserver')
    capital_route_express = db.Column(db.String(255), info='条件表达式')
    capital_route_create_at = db.Column(db.DateTime, info='创建时间')
    capital_route_update_at = db.Column(db.DateTime, info='更新时间')
    capital_route_index = db.Column(db.Integer, info='校验顺序')
...
class Card(db.Model, BaseToDict):
    ...
    card_individual_no = db.Column(db.String(64), nullable=False, index=True, server_default=db.FetchedValue(), info='持有人业务主键号')
    ...
    card_acc_id_num_encrypt = db.Column(db.String(128), index=True, info='账户身份证号码 密文')
    card_acc_tel_encrypt = db.Column(db.String(128), info='银行卡对应的电话号码 密文')
    card_acc_num_encrypt = db.Column(db.String(128), unique=True, info='银行卡UUID，非银行卡号')
    card_borrower_uuid = db.Column(db.String(128), info='借款人uuid 密文')
...
class CardBind(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('uk_card_bind_card_num_encrypt_channel_serial', 'card_bind_card_num_encrypt', 'card_bind_serial_no', 'card_bind_channel'),
        db.Index('unique_card_bind_card_num_channel_serial', 'card_bind_card_num', 'card_bind_serial_no', 'card_bind_channel'),
        db.Index('idx_card_bind_create_at', 'card_bind_serial_no', 'card_bind_create_at')
    )
    ...
    card_bind_serial_no = db.Column(db.String(50), nullable=False, server_default=db.FetchedValue(), info='绑卡序列号->对应代扣序列号')
    card_bind_channel = db.Column(db.String(32), nullable=False, server_default=db.FetchedValue(), info='绑卡通道')
    card_bind_status = db.Column(db.Enum('init', 'ready', 'process', 'success', 'fail'), nullable=False, server_default=db.FetchedValue(), info='绑定状态，init=>查询写入表记录; ready=>二次请求时修改为ready(不管成功还是失败！)；')
    card_bind_req_data = db.Column(db.Text)
    card_bind_res_data = db.Column(db.Text)
    card_bind_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue())
    card_bind_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue())
    card_bind_comment = db.Column(db.String)
    card_bind_card_num_encrypt = db.Column(db.String(128), nullable=False, server_default=db.FetchedValue(), info='绑卡卡号 密文字段')
...
class ComboOrder(db.Model, BaseToDict):
    ...
    combo_order_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
...
class ComboOrderTran(db.Model, BaseToDict):
    ...
    combo_order_tran_order_no = db.Column(db.String(48), nullable=False, index=True, info='订单编号')
    ...
    combo_order_tran_type = db.Column(db.String(32), nullable=False, server_default=db.FetchedValue(), info='订单明细类型')
    combo_order_tran_decrease_amount = db.Column(db.BigInteger, nullable=False, server_default=db.FetchedValue(), info='减免金额：分')
    combo_order_tran_repay_priority = db.Column(db.SmallInteger, nullable=False, server_default=db.FetchedValue())



class ConditionDefine(db.Model, BaseToDict):
    __tablename__ = 'condition_define'

    condition_define_id = db.Column(db.Integer, primary_key=True, info='主键')
    condition_define_code = db.Column(db.String(32), info='条件code')
    condition_define_name = db.Column(db.String(32), info='条件名称')
    condition_define_create_at = db.Column(db.DateTime, info='创建时间')
    condition_define_update_at = db.Column(db.DateTime, info='更新时间')
...
class Individual(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('udx_individual_idnum_name', 'individual_id_num_encrypt', 'individual_borrower_uuid'),
    )

    individual_id = db.Column(db.Integer, primary_key=True)
    individual_no = db.Column(db.String(64), nullable=False, index=True)
    ...
    individual_borrower_uuid = db.Column(db.String(128), info='姓名 密文')
    individual_id_num_encrypt = db.Column(db.String(128), info='身份证号码 密文')
    individual_tel_encrypt = db.Column(db.String(128), index=True, info='联系电话 密文')
...
class IndividualAsset(db.Model, BaseToDict):
    ...
    individual_asset_individual_no = db.Column(db.String(64), nullable=False, index=True)
...
class KeyValueAudit(db.Model, BaseToDict):
    __tablename__ = 'key_value_audit'
    __table_args__ = (
        db.Index('uniq_namespace_group_key_unique_key', 'namespace', 'group', 'key', 'unique_key'),
        db.Index('idx_namespace_group_key', 'namespace', 'group', 'key')
    )

    id = db.Column(db.Integer, primary_key=True, info='主键')
    namespace = db.Column(db.String(64), nullable=False, server_default=db.FetchedValue(), info='命名空间')
    group = db.Column(db.String(64), nullable=False, server_default=db.FetchedValue(), info='分组')
    type = db.Column(db.String(16), nullable=False, server_default=db.FetchedValue(), info='类型')
    key = db.Column(db.String(100), nullable=False, info='键')
    value = db.Column(db.String, info='值')
    key_value_id = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='kv主键')
    action = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='kv操作行为(1.创建成功待审核 2.修改待审核 3.删除待审核)')
    status = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='审核状态(0.待审核 1.审核成功 2.审核被拒绝 -10.保存)')
    memo = db.Column(db.Text, info='备注')
    created_by = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='创建人')
    reviewer = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='审核人')
    ...
    operate_info = db.Column(db.Text, info='操作描述')
    old_val = db.Column(db.String, info='修改前的历史值')
    version = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='版本号')
    unique_key = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='唯一键')
    updated_by = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue(), info='修改者')
...
class Provision(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('idx_provision_unique', 'provision_item_no', 'provision_tran_period', 'provision_tran_type', 'provision_type', 'provision_item_type'),
    )

    provision_id = db.Column(db.BigInteger, primary_key=True, info='拨备垫资主键')
    provision_type = db.Column(db.String(32), nullable=False, info='拨备垫资类型, arbitration=>仲裁；asset_void=>资产作废；asset_reverse=>资产冲正；clear_error=>清分错误；asset_void_refund=>资产作废退款')
    ...
    provision_tran_type = db.Column(db.String(50), nullable=False, info='交易类型')
    provision_tran_no = db.Column(db.String(64), nullable=False, index=True, server_default=db.FetchedValue(), info='关联的dtran/ftran_id 方便数据查询使用')
...
class RefundRequest(db.Model, BaseToDict):
    ...
    refund_request_card_num = db.Column(db.String(64), server_default=db.FetchedValue(), info='退款卡号')
    refund_request_channel = db.Column(db.String(32), nullable=False, server_default=db.FetchedValue(), info='退款通道')
...
class Sendmsg(db.Model, BaseToDict):
    ...
    sendmsg_create_at = db.Column(db.DateTime, nullable=False, index=True, server_default=db.FetchedValue(), info='消息创建时间')
...
class Sendsm(db.Model, BaseToDict):
    ...
    sendsms_id = db.Column(db.BigInteger, primary_key=True)
    sendsms_order_no = db.Column(db.String(64), nullable=False, index=True, server_default=db.FetchedValue(), info='短信业务编号')
    sendsms_type = db.Column(db.Enum('product', 'warn'), nullable=False, info='短信类型,product-产品类短信；warn报警类短信')
    sendsms_content = db.Column(db.String, nullable=False, info='短信内容')
    sendsms_send_response = db.Column(db.String, info='短信内容')
    sendsms_memo = db.Column(db.String(2048), nullable=False, server_default=db.FetchedValue(), info='备注')
    sendsms_send_channel = db.Column(db.String(32), nullable=False, server_default=db.FetchedValue(), info='发送渠道，如果为空，则走msgsvr通道发送,否则走对应的渠道发送短信,如美图')
    sendsms_status = db.Column(db.Enum('open', 'running', 'error', 'terminated', 'close'), nullable=False, info='状态')
    sendsms_next_run_at = db.Column(db.DateTime, nullable=False, index=True, server_default=db.FetchedValue(), info='下次发送时间')
    sendsms_create_at = db.Column(db.DateTime, nullable=False, index=True, server_default=db.FetchedValue(), info='创建时间')
    sendsms_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='更新时间')
    sendsms_version = db.Column(db.SmallInteger, nullable=False, server_default=db.FetchedValue(), info='版本')
    sendsms_priority = db.Column(db.SmallInteger, nullable=False, server_default=db.FetchedValue(), info='优先级')
    sendsms_retrytimes = db.Column(db.SmallInteger, nullable=False, server_default=db.FetchedValue(), info='重试次数')
...
class SplitOrderConfig(db.Model, BaseToDict):
    ...
    split_order_config_status = db.Column(db.Enum('valid', 'invalid'), nullable=False, server_default=db.FetchedValue(), info='状态')
...
class SplitOrderRule(db.Model, BaseToDict):
    ...
    split_order_rule_can_combo = db.Column(db.String(1), nullable=False, server_default=db.FetchedValue(), info='是否能够合并支付')
    ...
    split_order_rule_can_partly = db.Column(db.String(1), info='是否能够部分还款')
    split_order_rule_overdue = db.Column(db.String(1), info='逾期')
    ...
    split_order_rule_status = db.Column(db.Enum('valid', 'invalid'), nullable=False, server_default=db.FetchedValue(), info='状态')
    split_order_rule_same_company_combo = db.Column(db.Enum('Y', 'N'), nullable=False, server_default=db.FetchedValue(), info='同一签约主体是否能够合并')
    split_order_rule_same_channel_combo = db.Column(db.Enum('Y', 'N'), nullable=False, server_default=db.FetchedValue(), info='同一渠道是否能够合并')
...
class SplitOrderRuleCondition(db.Model, BaseToDict):
    ...
    split_order_rule_condition_status = db.Column(db.Enum('valid', 'invalid'), nullable=False, server_default=db.FetchedValue(), info='状态')
...
class SplitOrderRuleExpres(db.Model, BaseToDict):
    ...
    split_order_rule_express_value = db.Column(db.String(256), info='表达式取值')
    ...
    split_order_rule_express_status = db.Column(db.Enum('valid', 'invalid'), nullable=False, server_default=db.FetchedValue(), info='状态')
    split_order_rule_express_percent = db.Column(db.String(5), nullable=False, server_default=db.FetchedValue(), info='百分比')
...
class Synctask(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('idx_synctask_create_at', 'synctask_type', 'synctask_status', 'synctask_create_at'),
        db.Index('idx_synctask_type_key', 'synctask_type', 'synctask_key'),
        db.Index('idx_synctask_busi_key_date', 'synctask_busi_key', 'synctask_status', 'synctask_create_at', 'synctask_update_at')
    )
    ...
    synctask_busi_key = db.Column(db.String(255), nullable=False, server_default=db.FetchedValue())
    synctask_from_system = db.Column(db.String(50), nullable=False, info='任务来源系统')
    synctask_req_data = db.Column(db.String, info='任务数据，Json格式')
    synctask_res_data = db.Column(db.String, info='任务执行完车后，返回结果数据，Json格式')
    synctask_process_server = db.Column(db.String(50), nullable=False, server_default=db.FetchedValue())
...
class SystemDictionary(db.Model, BaseToDict):
    __tablename__ = 'system_dictionary'

    system_dictionary_id = db.Column(db.BigInteger, primary_key=True)
    system_dictionary_code = db.Column(db.String(64), nullable=False, unique=True, info='字典code')
    system_dictionary_name = db.Column(db.String(64), nullable=False, info='字典名称')
    system_dictionary_range_value = db.Column(db.Text, nullable=False, info='取值范围,值与值之间用","隔开')
    system_dictionary_status = db.Column(db.Enum('valid', 'invalid'), nullable=False, server_default=db.FetchedValue(), info='状态')
    system_dictionary_create_at = db.Column(db.DateTime, nullable=False, index=True, server_default=db.FetchedValue())
    system_dictionary_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue())
...
class TmpDataFix(db.Model, BaseToDict):
    ...
    tmp_data_fix_item_no = db.Column(db.String(50), nullable=False, index=True, info='数据修复资产编号')
    tmp_data_fix_tran_no = db.Column(db.String(50), nullable=False, unique=True, info='数据修复还款计划编号')
...
class TmpWithholdingAsset(db.Model, BaseToDict):
    __tablename__ = 'tmp_withholding_asset'

    tmp_withholding_asset_id = db.Column(db.BigInteger, primary_key=True)
    tmp_withholding_asset_item_no = db.Column(db.String(60), nullable=False, unique=True)
    tmp_withholding_asset_process_status = db.Column(db.Enum('nofinish', 'finish'), nullable=False, server_default=db.FetchedValue())
    tmp_withholding_asset_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue())
...
class Trade(db.Model, BaseToDict):
    ...
    trade_product_no = db.Column(db.String(64), nullable=False, server_default=db.FetchedValue())
...
class Withhold(db.Model, BaseToDict):
    ...
    withhold_serial_no = db.Column(db.String(64), nullable=False, unique=True, info='代扣流水号，业务主键')
    ...
    withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement', 'user_cancel'), nullable=False, server_default=db.FetchedValue(), info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付,user_cancel:用户取消')
    ...
    withhold_execute_at = db.Column(db.DateTime, info='执行时间')
    withhold_finish_at = db.Column(db.DateTime, index=True, info='代扣完成时间')
    ...
    withhold_card_num = db.Column(db.String(128), index=True, server_default=db.FetchedValue(), info='银行卡')
    withhold_user_uuid = db.Column(db.String(32), nullable=False, server_default=db.FetchedValue(), info='用户在用户中心ID')
    ...
    withhold_payment_mode = db.Column(db.String(256), server_default=db.FetchedValue())
    withhold_confirm_at = db.Column(db.DateTime, index=True, info='代扣确认时间')
    withhold_redirect_url = db.Column(db.Text, info='dsq跳转地址')
    withhold_user_ip = db.Column(db.String(256), info='用户id')
    withhold_payment_type = db.Column(db.String(256), info='支付产品')
    withhold_payment_option = db.Column(db.String(256), info='支付方式')
    withhold_payment_card_num = db.Column(db.String(256), server_default=db.FetchedValue(), info='用户确认的还款卡')
    withhold_payment_card_type = db.Column(db.String(32), info='用户还款账户类型')
...
class WithholdAssetDetailLock(db.Model, BaseToDict):
    ...
    withhold_asset_detail_lock_serial_no = db.Column(db.String(64), nullable=False, info='代扣序列号')



class WithholdAssetLock(db.Model, BaseToDict):
    __tablename__ = 'withhold_asset_lock'
    __table_args__ = (
        db.Index('uidx_asset_item_no_type', 'withhold_asset_lock_asset_item_no', 'withhold_asset_lock_operation_type'),
    )

    withhold_asset_lock_id = db.Column(db.Integer, primary_key=True, info='主键')
    withhold_asset_lock_asset_item_no = db.Column(db.String(50), nullable=False, info='资产编号')
    withhold_asset_lock_operation_type = db.Column(db.Enum('withhold'), nullable=False, info='锁类型')
    withhold_asset_lock_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
    withhold_asset_lock_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='更新时间')
...
class WithholdCard(db.Model, BaseToDict):
    ...
    withhold_card_from_app = db.Column(db.String(255), nullable=False, server_default=db.FetchedValue())
...
class WithholdDetail(db.Model, BaseToDict):
    ...
    withhold_detail_serial_no = db.Column(db.String(64), nullable=False, index=True, info='代扣流水号')
    ...
    withhold_detail_asset_tran_no = db.Column(db.String(32), nullable=False, info='资产明细编号')
...
class WithholdDetailHi(db.Model, BaseToDict):
    __tablename__ = 'withhold_detail_his'
    __table_args__ = (
        db.Index('idx_item_no_period', 'withhold_detail_asset_item_no', 'withhold_detail_period'),
    )

    withhold_detail_his_id = db.Column(db.Integer, primary_key=True, info='主键')
    withhold_detail_id = db.Column(db.Integer)
    withhold_detail_serial_no = db.Column(db.String(50), index=True, info='代扣流水号')
    withhold_detail_asset_item_no = db.Column(db.String(50), info='资产编号')
    withhold_detail_period = db.Column(db.Integer, info='期次')
    withhold_detail_asset_tran_type = db.Column(db.String(32), info='资产明细类型，根据asset_tran获取')
    withhold_detail_asset_tran_no = db.Column(db.String(50), info='资产明细编号')
    withhold_detail_asset_tran_amount = db.Column(db.Integer, info='资产应扣金额（单位:分）')
    withhold_detail_asset_tran_balance_amount = db.Column(db.Integer, info='资产剩余金额（单位:分）')
    withhold_detail_withhold_amount = db.Column(db.Integer, info='实际代扣金额（单位:分）')
    withhold_detail_type = db.Column(db.String(32), info='代扣明细分类，根据asset_tran而来')
    withhold_detail_partial = db.Column(db.Enum('Y', 'N'), server_default=db.FetchedValue(), info='是否部分还款')
    withhold_detail_status = db.Column(db.String(16), info='状态')
    withhold_detail_priority = db.Column(db.Integer, info='优先级')
    withhold_detail_create_at = db.Column(db.DateTime, index=True, server_default=db.FetchedValue(), info='创建时间')
    withhold_detail_update_at = db.Column(db.DateTime, index=True, server_default=db.FetchedValue(), info='更新时间')
    withhold_detail_repay_type = db.Column(db.Enum('advance', 'normal', 'overdue'), info='还款类型')
    withhold_detail_his_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
...
class WithholdHi(db.Model, BaseToDict):
    ...
    withhold_serial_no = db.Column(db.String(64), unique=True, info='代扣流水号，业务主键')
    withhold_request_no = db.Column(db.String(32), index=True, info='代扣请求编号')
    withhold_amount = db.Column(db.Integer, info='代扣金额（单位:分）')
    withhold_channel = db.Column(db.String(50), info='代扣渠道，根据不同的放款渠道进行路由')
    withhold_status = db.Column(db.Enum('ready', 'process', 'success', 'fail', 'cancel'), info='状态')
    withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement'), server_default=db.FetchedValue(), info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付')
    withhold_channel_code = db.Column(db.String(32), info='代扣渠道返回的code')
    withhold_channel_message = db.Column(db.String(255), info='代扣渠道返回的消息描述')
    ...
    withhold_error_code = db.Column(db.String(32), info='根据代扣渠道返回的状态自定义的code')
    withhold_custom_code = db.Column(db.String(32), info='自定义code')
    withhold_supplement = db.Column(db.Enum('Y', 'N'), server_default=db.FetchedValue(), info='是否为补单 Y N')
    withhold_order = db.Column(db.Integer, info='代扣顺序')
    withhold_third_serial_no = db.Column(db.String(64), info='代扣通道返回的流水号')
    withhold_extend_info = db.Column(db.Text, info='针对一些特殊处理保存数据')
    withhold_create_at = db.Column(db.DateTime, index=True, server_default=db.FetchedValue(), info='创建时间')
    withhold_update_at = db.Column(db.DateTime, index=True, server_default=db.FetchedValue(), info='更新时间')
    ...
    withhold_version = db.Column(db.Integer, info='版本号')
    withhold_req_key = db.Column(db.String(64), info='代扣请求key')
    withhold_user_name = db.Column(db.String(32), info='姓名')
    withhold_user_idnum = db.Column(db.String(32), info='身份证号')
    withhold_user_phone = db.Column(db.String(32), info='手机号')
    withhold_card_num = db.Column(db.String(32), info='用户卡号')
    withhold_capital_receive_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='资方接收时间')
    withhold_call_back = db.Column(db.String(500), info='回调地址')
    withhold_his_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
    withhold_confirm_at = db.Column(db.DateTime, info='代扣确认时间')
    withhold_redirect_url = db.Column(db.Text, info='dsq跳转地址')
    withhold_user_ip = db.Column(db.String(256), info='用户id')
    withhold_payment_type = db.Column(db.String(256), info='支付产品')
    withhold_payment_option = db.Column(db.String(256), info='支付方式')
    withhold_payment_mode = db.Column(db.String(256), info='支付方式')
...
class WithholdOrder(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('idx_serial_no_reference_no', 'withhold_order_serial_no', 'withhold_order_reference_no'),
    )
    ...
    withhold_order_withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement', 'user_cancel'), nullable=False, info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付,user_cancel:用户取消')
...
class WithholdOrderHi(db.Model, BaseToDict):
    __tablename__ = 'withhold_order_his'
    __table_args__ = (
        db.Index('uidx_serial_no_reference_no', 'withhold_order_serial_no', 'withhold_order_reference_no'),
    )

    withhold_order_his_id = db.Column(db.Integer, primary_key=True, info='主键')
    withhold_order_id = db.Column(db.Integer)
    withhold_order_request_no = db.Column(db.String(32), index=True, info='请求编号')
    withhold_order_serial_no = db.Column(db.String(64), info='代扣流水号')
    withhold_order_req_key = db.Column(db.String(64), index=True, info='请求key')
    withhold_order_amount = db.Column(db.Integer, info='订单金额（单位:分）')
    withhold_order_withhold_amount = db.Column(db.Integer, info='代扣金额（单位:分）')
    withhold_order_reference_no = db.Column(db.String(50), info='关联项目编号')
    withhold_order_withhold_status = db.Column(db.Enum('ready', 'process', 'success', 'fail', 'cancel'), info='状态')
    withhold_order_withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement'), info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付')
    withhold_order_operate_type = db.Column(db.Enum('active', 'auto', 'manual'), info='代扣操作类型')
    withhold_order_create_at = db.Column(db.DateTime, index=True, server_default=db.FetchedValue(), info='创建时间')
    withhold_order_update_at = db.Column(db.DateTime, index=True, server_default=db.FetchedValue(), info='更新时间')
    withhold_order_balance_amount = db.Column(db.Integer, info='剩余金额(单位:分)')
    withhold_order_his_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
...
class WithholdRepeated(db.Model, BaseToDict):
    ...
    __table_args__ = (
        db.Index('idx_itemno_tranype_peroid', 'withhold_repeated_asset_item_no', 'withhold_repeated_asset_tran_type', 'withhold_repeated_period'),
        db.Index('idx_serial_no_item_no', 'withhold_repeated_serial_no', 'withhold_repeated_asset_item_no')
    )

    withhold_repeated_id = db.Column(db.Integer, primary_key=True, info='主键')
    withhold_repeated_serial_no = db.Column(db.String(64), nullable=False, info='代扣流水号')
    withhold_repeated_asset_item_no = db.Column(db.String(50), nullable=False, info='关联编号')
    ...
    withhold_repeated_refund_serial_no = db.Column(db.String(50), server_default=db.FetchedValue(), info='退款序列号')
...
class WithholdRequest(db.Model, BaseToDict):
    ...
    withhold_request_user_name = db.Column(db.String(128), nullable=False, info='用户姓名')
    withhold_request_user_phone = db.Column(db.String(128), nullable=False, index=True, info='手机号')
    withhold_request_user_idnum = db.Column(db.String(128), nullable=False, info='身份证号')
    withhold_request_card_num = db.Column(db.String(128), server_default=db.FetchedValue(), info='银行卡')
    withhold_request_bank_code = db.Column(db.String(12), info='银行编码')
    ...
    withhold_request_from_system = db.Column(db.String(50), info='系统来源')
    withhold_request_trade_type = db.Column(db.String(32), nullable=False, info='交易类型')



class WithholdRequestHi(db.Model, BaseToDict):
    __tablename__ = 'withhold_request_his'

    withhold_request_his_id = db.Column(db.Integer, primary_key=True, info='主键')
    withhold_request_id = db.Column(db.Integer)
    withhold_request_no = db.Column(db.String(32), unique=True, info='业务主键，编号')
    withhold_request_req_key = db.Column(db.String(64), unique=True, info='请求key')
    withhold_request_operate_type = db.Column(db.Enum('active', 'auto', 'manual'), info='代扣操作类型：active  auto  manual')
    withhold_request_amount = db.Column(db.Integer, info='代扣请求金额（单位:分）')
    withhold_request_user_name = db.Column(db.String(32), info='用户姓名')
    withhold_request_user_phone = db.Column(db.String(32), info='手机号')
    withhold_request_user_idnum = db.Column(db.String(32), info='身份证号')
    withhold_request_card_num = db.Column(db.String(32), info='银行卡号')
    withhold_request_bank_code = db.Column(db.String(12), info='银行编码')
    withhold_request_status = db.Column(db.Enum('nofinish', 'finish', 'void'), info='状态,nofinish:未完成,finish:完成,void:作废')
    withhold_request_creator = db.Column(db.String(32), info='创建人')
    withhold_request_operator = db.Column(db.String(32), info='操作人')
    withhold_request_create_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='创建时间')
    withhold_request_update_at = db.Column(db.DateTime, server_default=db.FetchedValue(), info='修改时间')
    withhold_request_finished_at = db.Column(db.DateTime, info='完成时间')
    withhold_request_origin_req_key = db.Column(db.String(64), info='原始请求key')
    withhold_request_extend = db.Column(db.Text, info='保存短信验证码等信息')
    withhold_request_from_system = db.Column(db.String(32), info='系统来源')
    withhold_request_trade_type = db.Column(db.String(32), info='交易类型')
    withhold_request_his_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
...
class WithholdResult(db.Model, BaseToDict):
    ...
    __table_args__ = (
        ...
        db.Index('idx_withhold_result_useridnum_cardnum_encrypt', 'withhold_result_user_idnum_encrypt', 'withhold_result_card_num_encrypt')
    )
    ...
    withhold_result_user_name = db.Column(db.String(255), nullable=False, info='还款人姓名')
    withhold_result_user_phone = db.Column(db.String(32), nullable=False, info='还款人手机号')
    withhold_result_user_idnum = db.Column(db.String(32), nullable=False, server_default=db.FetchedValue(), info='还款人身份证号')
    withhold_result_card_num = db.Column(db.String(32), nullable=False, info='代扣卡号')
    withhold_result_bank_code = db.Column(db.String(32), nullable=False, info='代扣银行代码')
    withhold_result_status = db.Column(db.Enum('ready', 'success', 'fail', 'process', 'cancel'), nullable=False, index=True, info='代扣状态')
    ...
    withhold_result_owner = db.Column(db.String(24), nullable=False, server_default=db.FetchedValue(), info='代扣资产所有权者')
    withhold_result_user_name_encrypt = db.Column(db.String(128), info='还款人姓名 密文')
    withhold_result_user_phone_encrypt = db.Column(db.String(128), info='还款人手机号 密文')
    withhold_result_user_idnum_encrypt = db.Column(db.String(128), info='还款人身份证号 密文')
    withhold_result_card_num_encrypt = db.Column(db.String(128), info='代扣卡号 密文')
...
class WithholdResultTran(db.Model, BaseToDict):
    __tablename__ = 'withhold_result_tran'
    __table_args__ = (
        db.Index('idx_withhold_tran_asset_item_no', 'withhold_result_tran_asset_item_no', 'withhold_result_tran_asset_period'),
    )

    withhold_result_tran_id = db.Column(db.BigInteger, primary_key=True, info='自增长ID')
    withhold_result_tran_withhold_result_no = db.Column(db.String(20), nullable=False, index=True, info='代扣表业务主键：withhold_result表的withhold_result_no')
    withhold_result_tran_type = db.Column(db.Enum('principal', 'interest', 'fee', 'late'), nullable=False, info='代扣类型：principal/interest/fee')
    withhold_result_tran_asset_item_no = db.Column(db.String(50), nullable=False, info='资产编号')
    withhold_result_tran_asset_tran_no = db.Column(db.String(20), nullable=False, index=True, info='asset_tran业务ID')
    withhold_result_tran_asset_tran_type = db.Column(db.String(32), nullable=False, info='对应表asset_tran 的 asset_tran_type字段')
    withhold_result_tran_asset_period = db.Column(db.Integer, nullable=False, info='还款期次')
    withhold_result_tran_status = db.Column(db.Enum('finish', 'nofinish'), nullable=False, info='还款状态')
    withhold_result_tran_amount = db.Column(db.BigInteger, nullable=False, info='还款金额')
    withhold_result_tran_repay_type = db.Column(db.Enum('normal', 'advance', 'overdue'), nullable=False, info='还款类型：normal->正常还款,advance->提前还款,overdue->逾期还款')
    withhold_result_tran_create_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='创建时间')
    withhold_result_tran_update_at = db.Column(db.DateTime, nullable=False, server_default=db.FetchedValue(), info='更新时间')
