...
class XiaomizhongjiMock(BusinessMock):

    def __init__(self, project, asset, asset_extend, asset_tran_list, period_start, period_end):
        self.channel = 'xiaomi_zhongji'
        super(XiaomizhongjiMock, self).__init__(project, asset, asset_extend, asset_tran_list, period_start, period_end)
        self.trail_url = '/xiaomi/xiaomi_zhongji/04B177'
        self.trail_query_url = ''
        self.repay_plan_url = ''
        self.repay_apply_url = ''
        self.repay_apply_query_url = '/xiaomi/xiaomi_zhongji/04B179'
    ...
    def repay_trail_mock(self, status, principal_over=False, interest_type='less'):
        ...
        if interest_type == 'less':
            ...
            interest_amount = 0
        value = dict(zip(('$.data.repayPrin', '$.data.repayInterest'),
    ...
    def repay_apply_query_mock(self, withhold, withhold_detail, success_type='success'):
        ...
        value = dict(zip(('$.data.amtPaid', '$.data.prinPaid', '$.data.interestPaid', '$.data.repayStatus',
                          '$.data.repayDate'), (
            self.fen2yuan(principal_amount + interest), self.fen2yuan(principal_amount), self.fen2yuan(interest),
            status, self.get_date(fmt='%Y-%m-%d', is_str=True))))
