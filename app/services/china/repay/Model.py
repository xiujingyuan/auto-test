...
class Withhold(db.Model, BaseToDict):
    ...
    withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement', 'user_cancel', 'offline'), nullable=False, server_default=db.FetchedValue(), info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付')
...
class WithholdHi(db.Model, BaseToDict):
    ...
    withhold_channel = db.Column(db.String(50), info='代扣渠道，根据不同的放款渠道进行路由')
    withhold_status = db.Column(db.Enum('ready', 'process', 'success', 'fail', 'cancel'), nullable=False, info='状态')
    withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement', 'user_cancel'), nullable=False, server_default=db.FetchedValue(), info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付')
    withhold_channel_code = db.Column(db.String(32), info='代扣渠道返回的code')
    withhold_channel_message = db.Column(db.String(255), info='代扣渠道返回的消息描述')
...
class WithholdOrder(db.Model, BaseToDict):
    ...
    withhold_order_withhold_sub_status = db.Column(db.Enum('normal', 'payment_cancel', 'agreement', 'user_cancel', 'offline', 'asset_delay'), nullable=False, info='子状态,normal:正常,payment_cancel:协议支付取消,agreement:协议支付')
