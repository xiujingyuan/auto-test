...
class MockConfig(db.Model):
    ...
    mock_config_jsonpath = db.Column(db.String(500), nullable=False, server_default=db.FetchedValue(), info='需要替换值的描述')
    mock_config_order = db.Column(db.Integer, default=100, info='mock显示顺序')
    mock_config_is_collect = db.Column(db.Integer, default=0, info='mock是否喜爱')
    mock_config_is_active = db.Column(db.Integer, default=1, info='mock是否激活')
    mock_config_timeout = db.Column(db.Integer, default=30, nullable=False, info='替换超时时间')
